...
<html>
  ...
  <body>
    <div class="wrapper">
      ...
      $conn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD) or die('<div class="error">Could not connect to the database</div>');
      mysqli_select_db($conn, DB_NAME) or die('<div class="error">Could not select the database</div>');
     ...
     ?>
      ...
      <?php
        ...
        while($row = mysqli_fetch_assoc($result))
        {
          ...
          $update = "UPDATE `" . $table_prefix . "posts` SET post_content = '" . mysqli_real_escape_string($conn, $new_content) . "' WHERE ID = '" .  $id ."'";
          $result2 = mysqli_query($conn, $update);
          if($result2)
          {
